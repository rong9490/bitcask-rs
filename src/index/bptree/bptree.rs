...
use super::super::super::data::log_record::utils::decode_log_record_pos;
...
impl BPlusTree {
    pub fn new(dir_path: PathBuf) -> Self {
        let whole_path: PathBuf = dir_path.join(BPTREE_INDEX_FILE_NAME);
        // 打开B+树实例, 创建对应的bucket
        let bptree: DB = DB::open(whole_path).expect("failed to open bptree");
        let tree = Arc::new(bptree);
        // HINT 后续熟悉其操作
        let tx: Tx<'_> = tree.tx(true).expect("failed to begin tx");
